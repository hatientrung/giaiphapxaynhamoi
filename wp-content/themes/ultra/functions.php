...
function ultra_scripts() {
	...
	// TRUNG HA
	wp_enqueue_style( 'ultra-services', get_template_directory_uri() . '/css/custom-by-trungha.css' );
...
function ultra_remove_current_menu_class( $classes, $item, $args, $depth ) {
...
add_filter( 'nav_menu_css_class', 'ultra_remove_current_menu_class', 10, 4 );
...
function ultra_render_slider() {
	if ( is_front_page() && ! in_array( siteorigin_setting( 'home_slider' ), array( '', 'none' ) ) ) {
		...
		if ( ( is_page() || $is_wc_shop ) && get_post_meta( $page_id, 'ultra_metaslider_slider', true ) != 'none' ) {
			...
			if ( $slider_overlap === '' ) {
				// We'll default to whatever the home page slider overlap setting is.
				$slider_overlap = siteorigin_setting( 'home_header_overlaps' );
			}					
		}
